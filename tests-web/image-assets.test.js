...
test("edited Bedrock images return to the same nested path and container", async () => {
  ...
  const output = await JSZip.loadAsync(result.archive, { checkCRC32: true });
  assert.equal(output.file("packs/UI.mcpack"), null);
  const nested = await JSZip.loadAsync(
    await output.file("UI.mcpack").async("uint8array"),
    { checkCRC32: true },
  );
